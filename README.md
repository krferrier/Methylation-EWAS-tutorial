...
**Rendered tutorial:** https://krferrier.github.io/EWAS-tutorial/

## The dataset

Grady Trauma Project, NCBI GEO accession **GSE132203** — whole-blood EPIC methylation
with matched phenotype data. The walkthrough uses a curated subset of **n = 87**
(32 PTSD cases, 55 controls; approximately 95% African American). After QC and
filtering under the Zhou EPIC `mask.cm` **v8.1** recommended mask,
**756,251 CpGs** are tested.
...
| 03 | `03_probe_filtering.qmd` | Zhou v8.1 mask, detection *p*, sex chromosomes |
...
The tutorial tests the full post-QC CpG set rather than a 20,000-probe subsample, so the
intermediate objects are large: the normalized `GenomicRatioSet` alone is 1.2 GiB and the
whole checkpoint set is about 4.3 GiB on disk. That does not belong in a git repository,
and git-lfs is a poor fit — the quota is small, metered, and shared across a user's
repositories, so a tutorial that students clone repeatedly would consume it quickly.

Instead: **code, prose and every file under 5 MB are in git; the large checkpoints are a
...
outputs so the chapter renders without a 4-hour compute.
