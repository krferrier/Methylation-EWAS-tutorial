...
[@chen2013crossreactive] catalogued cross-reactive probes on the 450K; McCartney
...
`MASK_typeINextBaseSwitch` in the archived manifest. Be aware that the current v8.1
mask retains only the extension-base category (as `M_1baseSwitchSNP_*`) and drops the
repeat and copy-number terms entirely; see the note in §2.

The latest CpG masking recommendations from Zhou (`EPIC.hg38.mask.cm`, v8.1) are
distributed as **24 separate mask terms**, each a one-bit-per-probe track, rather
than as one wide table of boolean columns. One of those terms is `M_general`, which
...
their union. That is what makes the admixed case in §3 expressible at all. The full
set of 24 for EPIC is:
...
::: {.callout-warning}
## v8.1 does not mask repeats or copy-number regions
The legacy manifest's `MASK_general_<POP>` bundled `MASK_rmsk15` (RepeatMasker),
`MASK_sub25_copy` (copy-number-variable segments) and `MASK_typeINextBaseSwitch`
together with the mapping and SNP flags. **v8.1 has no repeat or copy-number term
among its 24.** This is why the v8.1 mask is the *less* aggressive of the two on
this cohort despite covering two ancestries — quantified at the end of this section.
If excluding repeat-overlapping probes matters for your question, you need the
archived manifest or a separate repeat annotation; it is not in `mask.cm`.
:::
...
`yame summary` is the quickest way to see what a term costs you before you commit
...
SNP" depends on which population you ask about. Zhou's latest masking manifest 
gives masking information for the general population as well as the five super-populations 
from the 1000 Genomes project. The archived population manifest gives a
`MASK_snp5_<POP>` (SNP within 5 bp) flag for each of 31 1000 Genomes populations (5 super-populations 
and 26 sub-populations). The number of SNP-affected probes varies substantially by ancestry:
...
AFR versus **`r format(funl$snp_by_pop[["EUR"]], big.mark = ",")`** for EUR, and the
two lists are far from nested:
...
This is the concrete argument for taking a union rather than picking one
population's mask: neither list contains the other.
...
So, if we are using the latest recommended masking file, the columns we would want 
...
::: {.callout-note}
## Probes you might filter for a different reason: smoking
...
belong in the funnel above. This tutorial does not take that route; see
[the smoking section in the EWAS chapter](06_ewas.qmd) for the full set of options
and why surrogate variables are used here instead.
:::
...
**756,251** are actually tested. There is no other subsetting downstream — in
particular, no random subsampling: the EWAS runs on the whole matrix.
