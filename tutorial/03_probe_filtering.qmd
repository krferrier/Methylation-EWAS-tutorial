...
# This is how data/03_legacy_pop_mask.tsv.gz was built
# from the archived manifest. That manifest is not in the repository -- the
# shipped file is its output, and the chunk below reads it.
library(data.table)
...
The mapping criteria is where the majority of the difference in count for recommended
probe masks come from. The legacy mask flagged 65,537 probes as mapping failures and
another 25,194 for a non-unique 30-base 3′ subsequence; v8.1 flags 428 under
...
criteria for 70,632 probes, whereas v8.1 that decreased to 29,261.
