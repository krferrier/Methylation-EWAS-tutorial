...
[§2](#sec-categories) needs and the TSV does not have. Start with the TSV; read the
...
This is where the five-population table in [§2](#sec-categories) comes from, and it is
the concrete reason the chapter needs both forms of the mask. The TSV carries
`M_SNP_AFR_1pt` and `M_SNP_EUR_1pt` — which is why [§3](#sec-ancestry) can draw its
AFR-versus-EUR contrast straight from it — but it does not carry the AMR, SAS or EAS
equivalents, so three of that table's five rows can only come from the full release. Because the unpacked bits are committed, the count runs here rather
