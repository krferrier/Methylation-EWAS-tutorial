...
difference is really a genotype difference). These are 
...
This chapter starts from `data/02_funnorm_grs.rds`, written by the save step in
...
# The functional-normalized GenomicRatioSet saved by
...
There are two main types of problematic probes, each characterized in dedicated
methods papers:
...
et al. [@mccartney2016identification] extended this to the EPIC array. In Zhou's
v8.1 masks this is two flags: `M_mapping`, for probes that do not map at all, align
below score 35, or (for Infinium-I) whose two alleles land in different places; and
`M_nonuniq`, for probes that map but with mapping quality under 10 — which is the one
that catches a probe hybridising in several locations.
...
SNP mask recommendations built from 1000 Genomes allele frequencies. If you are studying a 
population that is not well represented by one of the 1000 Genomes Project populations, it is 
...
# -d . puts the files in ./EPIC/ in the current directory. Without it they go to
# YAME's shared store (~/.local/share/yame by default), and the commands below
# would need the full path to that folder.
yame fetch -d . EPIC/EPIC.hg38.mask.cm EPIC/EPIC.ordering.tsv.gz

yame info    EPIC/EPIC.hg38.mask.cm   # 24 mask terms over 866,553 probes
yame summary EPIC/EPIC.hg38.mask.cm   # how many probes each term flags
yame unpack -a EPIC/EPIC.hg38.mask.cm > mask_bits.tsv   # all 24 terms, one column each
```

`yame summary` is the quickest way to see what a term includes before you commit
...
library(data.table)
# The mask's term names are in the .idx file `yame fetch` downloaded alongside
# it, one per line, in the same order as the columns of mask_bits.tsv (which has
# no header).
...
population's mask. For an admixed cohort with a few known ancestral components (as here, AFR + EUR),
the conservative choice is to take the **union** of the relevant population masks (drop
a probe if it is SNP-affected in *any* represented ancestry). For a cohort spanning many
ancestries, or of unknown ancestry, the general population mask is the most appropriate.
:::
...
- **Detection-p failures.** Detection p-values were introduced in [QC](01_qc.qmd),
  which summarised them per sample; here they are recomputed per probe. A probe that fails (p > 0.01) in more than ~10% of samples is
...
knitr::kable(funnel, caption = "Probe-filtering funnel on the 96-sample subset (AFR + EUR aware). Each filter is counted on its own and a probe can fail more than one, so the dropped counts do not add up to the total removed.")
...
The two generations do not mask the same things, and the legacy pair masks more.
...
```{.r filename="Provenance, do not run"}
# This is how data/03_legacy_pop_mask.tsv.gz was built from the archived
# manifest (14 MB, read from the zhou-lab repository). The shipped file is its
# output, and the chunk below reads it.
library(data.table)
pop  <- fread(paste0("https://raw.githubusercontent.com/zhou-lab/InfiniumAnnotationData/",
                     "6d176d6e7e56465403d4478a09fe11b983eeb721/Anno/EPIC/EPIC.hg38.manifest.pop.tsv.gz"),
              select = c("probeID", "MASK_general_AFR", "MASK_snp5_EUR"))
long <- rbindlist(lapply(c("MASK_general_AFR", "MASK_snp5_EUR"),
...
# which is how two columns of an 866k-probe annotation fit in 0.6 MB.
...
The population SNP masks are slightly different between v8.1 and the legacy masking,
but overall are quite consistent. Both generations flag a SNP of at least 1% frequency
within 5 bp of the 3′ extension, so `MASK_snp5_AFR` and v8.1's three AFR SNP terms are
the same criterion: 59,361 probes against 55,405. What differs is mostly *which*
probes — 7,110 flagged only by the legacy columns, 3,154 only by v8.1. These
differences are largely because a new aligner and genome build were used in v8.1; a
probe's coordinates and therefore its 5 bp window could have shifted, as well as
updated data on population-specific allele frequencies which are used to define SNPs.

The mapping criteria are where most of the difference in the recommended mask counts
comes from. The legacy mask flagged 65,537 probes as mapping failures and 25,194 for a
non-unique 30-base 3′ subsequence, many of them the same probes; v8.1 flags 428 under
`M_mapping` (unmapped, alignment score below 35, or Infinium-I alleles landing in
different places) and 28,833 under `M_nonuniq` (mapped, but mapping quality under 10).
v8.1 has no separate copy-number term because mapping quality now captures that
information. In total that means the legacy code recommended masking for mapping
criteria for 70,632 probes, whereas in v8.1 that number is 29,261.

Given the vast difference in mapping-related criteria between versions, it's
recommended to use the current `M_mapping` and `M_nonuniq` and only use the population
specific columns (`MASK_snp5_<POP>`) from the legacy version if you need sub-population
SNP masking. And, make sure to record which columns from which versions of the masking
...
2. **Apply the same masks by ID.** Read Zhou's mask/manifest for the correct array
   ...
   # archived manifest (14 MB), if you need sub-population resolution instead
   library(data.table)
   pop  <- fread(paste0("https://raw.githubusercontent.com/zhou-lab/InfiniumAnnotationData/",
                        "6d176d6e7e56465403d4478a09fe11b983eeb721/Anno/EPIC/EPIC.hg38.manifest.pop.tsv.gz"))
...
manifest, then intersect on the shared probe IDs. This tutorial uses the hg38 masks
because its results are reported in hg38 coordinates ([annotation](08_annotation.qmd));
the object's hg19 Illumina annotation is used here only to find the sex-chromosome
probes.
:::
...
```{r}
#| label: save-filtered
# 1. The EWAS-ready GenomicRatioSet. An S4 object with two parallel assay matrices
#    (Beta and CN; getM() derives M-values from Beta) and a GRanges, so RDS rather than a flat file. Read by cell-composition
...
The funnel counts are saved on their own too, in `data/03_filter_funnel.tsv`, which is
committed to the repository, so you can still read them without running the chapter.
:::
...
- **two probes** (`cg17759086`, `cg01801182`) have β pinned at 0 or 1 in the
  86-sample analysis subset, so their M-values are ±∞ and they cannot be fitted.
