...
# The same filter with one ancestry's SNP codes instead of both, for the contrast
# below. M_mapping and M_nonuniq are not ancestry-specific, so they stay in every
# variant; detection-p and sex filters are held fixed too. Only the SNP codes move.
one_anc <- function(pop) {
  cols <- c("M_mapping", "M_nonuniq",
            paste0(c("M_SNP_", "M_1baseSwitchSNP_", "M_2extBase_SNP_"), pop, "_1pt"))
  dropped <- mask$Probe_ID[Reduce(`|`, lapply(cols, function(m) mask[[m]] == 1L))]
  in_study <- intersect(rownames(grs), dropped)
  list(masked   = length(in_study),
       retained = length(setdiff(rownames(grs),
                                 Reduce(union, list(in_study, drop_detp, drop_sex)))))
}
afr_only <- one_anc("AFR")
eur_only <- one_anc("EUR")
keep_eur <- setdiff(rownames(grs),
                    Reduce(union, list(intersect(rownames(grs),
                             mask$Probe_ID[Reduce(`|`, lapply(
                               c("M_mapping", "M_nonuniq", "M_SNP_EUR_1pt",
                                 "M_1baseSwitchSNP_EUR_1pt", "M_2extBase_SNP_EUR_1pt"),
                               function(m) mask[[m]] == 1L))]),
                                       drop_detp, drop_sex)))
...
funl <- list(
  ...
  retained_eur     = length(keep_eur),                 # under EUR SNP codes only
  afr_only         = afr_only,                         # masked / retained, AFR codes only
  eur_only         = eur_only,                         # masked / retained, EUR codes only
...
#| tbl-cap: "Probes masked and retained when only the SNP codes change. `M_mapping` and `M_nonuniq` are in all three variants, and the detection-p and sex-chromosome filters are held fixed, so the rows differ only in which ancestry's SNP flags are applied."
anc <- data.frame(
  mask = c("AFR SNP codes only", "EUR SNP codes only",
           "AFR + EUR union (used here)"),
  probes_masked   = c(funl$afr_only$masked,   funl$eur_only$masked,   funl$drop_mask),
  probes_retained = c(funl$afr_only$retained, funl$eur_only$retained, funl$retained)
)
knitr::kable(anc, row.names = FALSE,
             format.args = list(big.mark = ","),
             col.names = c("SNP codes applied", "probes masked", "probes retained"))
```

A EUR-only mask retains `r format(funl$afr_not_eur_kept, big.mark = ",")` probes that
the union removes. Those are the probes carrying a common SNP in African-ancestry
individuals but not in Europeans — exactly the ones that would be left in this cohort
by the common default.
...
The difference is the repeat and copy-number terms described in §1: the legacy
