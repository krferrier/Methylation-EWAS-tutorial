...
computed here and explained in [§3](#sec-detp-sex).
...
# object -- removed for an autosomal EWAS, see §3.
...
# in §2. M_mapping and M_nonuniq are not ancestry-specific, so they stay in every
...
# Per-population SNP counts, written by the `snp-by-pop` chunk above and read
# back here so the funnel and the §2 table quote one source.
...
## 4. When you don't have IDATs: filtering a processed matrix {#sec-no-idats}
