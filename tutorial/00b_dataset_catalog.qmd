...
# --- Genetic PCs: deposited as sample characteristics, so read every field ----
# Submitters name them "pc1", "ancestry_pc1", "anc.pca1" or "snp based principle
# component 1", so match all four spellings.
pc_pat <- "^(ancestry[_ ]?)?pc[0-9]+$|^anc\\.?pca?[0-9]+$|princip(al|le) component"
cat70$Has_genetic_PCs <- vapply(cat70$All_char_tags, function(x) {
  f <- tolower(trimws(strsplit(ifelse(is.na(x), "", x), "|", fixed = TRUE)[[1]]))
  any(grepl(pc_pat, f))
}, logical(1))
...
show_cols <- c("Accessions", "Title", "Phenotypes", "Suitability_tier",
               ...
               "Has_age", "Has_ancestry", "Has_genetic_PCs", "Has_cell_composition",
               "N_phenotypes", "PubMed")
...
csv_cols <- c("GSE", "N_series", "Title", "Phenotypes", "N_phenotypes",
              ...
              "Has_sex", "Has_age", "Has_ancestry", "Has_genetic_PCs", "Has_cell_composition", "PubMed",
...
pretty <- c(Accessions = "GEO accession", Title = "Title",
            ...
            Has_ancestry = "Has ancestry", Has_genetic_PCs = "Has genetic PCs",
    ...
    options = list(
      ...
      columnDefs = list(
        ...
        list(targets = idx(c("Suitability_tier", "Suitability_score", "N_samples",
                             ...
                             "Has_genetic_PCs", "Has_cell_composition", "PubMed")),
...
| `Has genetic PCs` | Whether genetic ancestry principal components are deposited with the samples, for use as model covariates (see [Genetic ancestry PCs](04b_genetic_pcs.qmd)). |
...
against. There are also published manuscripts that we can compare our results with [@smith2011grady] [@katrinli2024ptsd].
...
above, in the same order, with `r length(csv_cols)` columns rather than the `r length(show_cols)` shown:
...
**Next:** with a dataset in hand, head to [Background](00a_background.qmd) to meet the
platform — or, if you already know the array, skip straight to [Setup](00_setup.qmd)
to download the raw IDATs and read them into R.
