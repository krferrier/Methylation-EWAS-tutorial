...
plm <- reshape(pl, varying = colnames(props), v.names = "prop",
               ...
               direction = "long")
# Stacking order follows the factor order, so set it explicitly. ggplot2 draws
# the first level at the top of each bar, so neutrophils form the top band and
# NK cells the bottom one.
...
2. **`EpiDISH` (reference-based, matrix-native).** The `EpiDISH` package
   ...
   ```{.r filename="Generic example, do not run"}
   # Schematic only. `beta_mat` stands for your own CpG x sample beta matrix. The
   # result is deliberately not called `props`: that name holds this chapter's IDOL
   # estimates, and overwriting it would quietly change every figure and model
   # after this point.
   library(EpiDISH)
...
[@houseman2014reffree], SVA [@leek2012sva], and RUV. This tutorial does not use
RefFreeEWAS; SVA is used in [batch effects](05_batch_effects.qmd) to capture unmeasured
variation, and RUV is mentioned there. The trade-off is that the recovered
...
([chapter 01](01_qc.qmd)) drop out later, when the exposure enters the model, as does the one sample with
no genetic ancestry PCs ([Genetic ancestry PCs](04b_genetic_pcs.qmd)), leaving
n = 86 for the EWAS.

---

**Next:** [Genetic ancestry PCs](04b_genetic_pcs.qmd) — what the three ancestry
covariates used from chapter 05 onwards are, and why they are a better covariate than
self-reported race.
