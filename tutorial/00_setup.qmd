...
## Building the software environment
...
## Downloading the 96-sample subset
...
## Computational cost: time, memory, and disk
...
| [Chapter 07](07_pipeline.qmd) | Non-stratified EWAS on 87 samples            | 6 min 16 s | 2.3 GB |
| [Chapter 07](07_pipeline.qmd) | Stratified EWAS: 2 strata (45 and 42) run simultaneously | 11 min 53 s | 6.3 GB |
...
those two in parallel, which is why the memory roughly triples even though each model
is smaller. If you're limited by RAM, you can run each part sequentially instead of in 
parallel so the max memory needed will be less than that of the single EWAS estimate. The amount of 
memory required for the EWAS using snakemake can be further reduced by reducing the number of workers. 
The trade-off for reducing the number of workers is that the wall time will increase. 
...
snakemake in Chapter 07, but requires more memory than the EWAS run on the same sample size (the above paragraph explains 
the higher memory requirements for a stratified analysis). 87 samples is quite small, and a full-scale EWAS will have 
...
## Optional: the full 795-sample dataset
