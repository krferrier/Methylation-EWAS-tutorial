...
That makes this a genuine end-to-end run rather than a toy demonstration: the
combined arm takes roughly two hours on a busy workstation, and every number and
figure below is the pipeline's own output.
...
### Carrying the surrogate variables and the smoking proxy across

Two of the covariates in `pheno.csv` were *constructed* in the earlier chapters
rather than measured, and they need care when you hand them to the pipeline.

**Surrogate variables.** Chapter 05 estimated six SVs with `sva` on the
ComBat-adjusted matrix and chapter 06 fit them as fixed covariates. The pipeline
has no SVA step — by design. It reads covariate *columns*, so the SVs travel as
six plain numeric columns `SV1`–`SV6`, computed once upstream and frozen. That is
the only way to keep the combined arm identical to chapter 06's model.

It also has a consequence you should be aware of when you stratify. The SVs were
estimated **once, on the whole cohort**, protecting `PTSD`. When the pipeline
splits by sex it carries those same whole-cohort columns into each stratum
unchanged; it does not — and with this design cannot — re-estimate SVs within
each arm. So inside a single-sex stratum the SV columns still encode structure
that was partly defined by sex differences across the full cohort. They remain
useful as technical-variation controls, but do not read a per-stratum SV
coefficient as though it were estimated in that stratum.

**The smoking proxy.** `smoke` is PC1 of 20 replicated smoking-associated CpGs
(chapter 05), oriented so that higher values mean heavier exposure. Because it is
derived from methylation itself, the 20 panel probes were **excluded from the
tested probe set** before the matrix was written — otherwise those CpGs would be
regressed on a linear combination of themselves. Two further probes drop out
because their β-values are pinned at 0 or 1 in this 87-sample subset, making
log₂(β/(1−β)) non-finite. That is why the pipeline reports 756,251 CpGs rather
than the 756,273 that survived filtering.
...
This writes the **full filtered probe set** — every CpG that survived Chapter 3 —
not a subset. The chunking machinery is what makes that affordable: `ewas.R` reads
...
Wall-clock times for this run, on 32 cores with `--workers 4`:

| Arm | *n* | CpGs | Wall time |
|---|---|---|---|
| Combined | 87 | 756,251 | 1 h 54 m |
| Female | 45 | 756,251 | 1 h 55 m |
| Male | 42 | 756,251 | 1 h 03 m |
| BACON (each arm) | — | 756,251 | ~23 s |
| METAL meta-analysis | — | 756,251 | ~90 s |

Treat these as an order-of-magnitude guide, not a benchmark. The three EWAS arms
were run concurrently on a shared machine, and each `glm` call spawns its own
multi-threaded BLAS, so the node was heavily oversubscribed (load average 190–250
on 32 cores) throughout. A single arm run alone, with BLAS threading pinned to
one thread per worker, is considerably faster. The lesson to take is the *shape*
of the cost: the per-CpG regressions dominate, and everything downstream —
BACON's 5,000 Gibbs iterations, METAL's two passes over three-quarters of a
million markers — is small change by comparison.
:::
...
1.014 / 1.013 — which is the behaviour you want from it: a small, symmetric
...
::: {.callout-important}
## BACON is not a licence to ignore λ
BACON corrects the *distribution* of test statistics; it does not manufacture
signal. If your λ is inflated because of unmodelled batch or cell composition,
...
1. **Check the arms.** For every hit you plan to report, look up the per-stratum
   ...
   finding at best, and you should describe it that way.
...
For the purposes of this tutorial, we ran the compare the EWAS without stratification, 
...
![Per-CpG PTSD effect sizes, female vs male stratum. Grey = not meta-significant; coloured points are the meta-analysis top hits.](data/07_effect_concordance.png){#fig-concordance width=70%}

A few caveats about these results:

- **The combined arm finds one CpG at Bonferroni; the stratified arms and the
  meta-analysis find more, and that asymmetry is instructive rather than
  encouraging.** The single-sex arms have ~40 samples each and consequently much
  larger standard errors (median SE 0.078 in females and 0.089 in males, against
  0.050 in the combined arm). Wider sampling distributions produce more extreme
  tail statistics by chance, so a stratified design at this size buys apparent
  hits, not power. Compare the four QQ curves in @fig-qq-overlay: the two strata
  and the meta-analysis all sit above the combined arm across the whole range.
- **The female/male effect concordance is essentially zero** (Pearson r = −0.004
  genome-wide, with sign agreement at 49.9% — indistinguishable from a coin flip;
  @fig-concordance). At genome scale that is the *correct* result: almost no CpG
  carries a real PTSD effect, so the two stratum estimates are independent noise.
  It also means a scatter of stratum effects can never, on its own, tell you
  whether a specific CpG replicated — which is what the formal meta-analysis and
  its per-hit diagnostics are for.
- **Heterogeneity sits at 5.4%** of CpGs below P = 0.05, against a 5% null
  ...
  sex-differential methylation genome-wide. In a real study an *excess* here would
  be the signal that sex-specific effects deserve dedicated investigation. Note
  the tension with the hit table above, where most of the nine top CpGs show high
  I²: heterogeneity is undetectable *on average* and rampant *among the extremes*,
  which is exactly the pattern winner's-curse produces at this sample size.
- **The honest summary of this run is a null result.** The point of the chapter is
