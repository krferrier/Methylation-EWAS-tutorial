...
filtered methylation matrix ([P2](02_normalization.qmd)–[P3](03_probe_filtering.qmd)),
cell-composition estimates ([P4](04_cell_composition.qmd)), and surrogate variables
that absorb technical structure ([P5](05_batch_effects.qmd)). This notebook fits the
association model, checks that it is well-calibrated, and interprets the results
honestly. It also lays out **two modeling conventions** you will meet in the
...
methylation change." We follow this throughout: fit on M, report Δβ. It is 
becoming more common to only report M-values, but it's still useful to generate both.
:::

## 2. The covariate set

Our design protects everything we established as a real driver of methylation:

- **sex** and **age** — strong, ubiquitous methylation covariates
  ([P5](05_batch_effects.qmd) confirmed both drive leading PCs).
- **cell composition** (6 estimated proportions) the single largest source
  of variance in whole blood ([P4](04_cell_composition.qmd)).
- **a smoking proxy** — this cohort has no recorded smoking variable, so
  [P5](05_batch_effects.qmd) builds one from a published panel of replicated
...
- **surrogate variables** — the `r ew$k_sv` SVs retained in
  [P5](05_batch_effects.qmd), which absorb structure that none of the measured
  covariates name.

Chip (slide) is *not* in this list. It is 87% collinear with sex in this subset, so
`~ ptsd + sex + slide` is not estimable; [P5](05_batch_effects.qmd) removes chip
effects with ComBat inside each sex stratum before the model is fitted. The matrix
entering `lmFit` below is that ComBat-adjusted matrix.

::: {.callout-note}
## Smoking 
Another covariate that is important to account for in modeling is smoking. Smoking is 
well established to cause distinct, systemic methylation changes, so it is important to include 
...
to smoking if you don't have a measured smoking variable included in the null model, but use this 
...
```r
library(limma)
# Mcb = ComBat-adjusted M-values from P5, probes x samples
# smoke = smoking proxy (P5); pos = array position factor; SV = surrogate variables
...
null. This corrects both **bias** (a non-zero centre of the null — a systematic shift) and
...
- A λ near 1 does not mean there is *no* residual bias — bias (a shift of the null's
  centre) barely moves λ, which is driven by the *spread* of the statistics. BACON
...
BACON works from **effect sizes and their standard errors**, which limma gives us
directly (SE = logFC / *t*):
...
result that a study of this size cannot settle either way. With n = 87 and a subtle
psychiatric phenotype, the sensible reading is that nothing here is established: a
single marginal hit in 756,251 tests is exactly what an underpowered scan produces,
and it would need replication in an independent cohort before it meant anything.
...
::: {.callout-note}
## In this dataset the df argument does not actually bind
It is worth checking the premise rather than assuming it. [P5](05_batch_effects.qmd)
compares the model above against the same model with the SV block removed: dropping the
`r ew$k_sv` SVs frees 6 df, and the *median standard error of the PTSD coefficient rises
by about 11%*. The SV block reduces each probe's residual variance by more than it costs
in df, so at this k the trade-off runs the other way from the textbook description. The
residualization workflow is still worth knowing — it is what many pipelines do, and the
df savings are real when the nuisance block is large — but "spends df, therefore costs
power" is a claim to test on your own data, not a rule.
...
methylation effects are common, I strongly recommond performing sex-stratified analyses.
...
subtype, exposure dose, or age band can all justify it. The cost is power — each stratum
...
- Obtain covariates from the series metadata: sex and age from `pData`, cell
  ...
  ([P4](04_cell_composition.qmd) covers this), and surrogate variables from
  `sva` run on the matrix ([P5](05_batch_effects.qmd)).
