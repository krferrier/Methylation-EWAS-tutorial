...
bio <- model.matrix(~ exposure + sex + age + cell_proportions, data = pheno)
...
managed workflow. METAL's `SCHEME STDERR` *is* an inverse-variance-weighted fixed-effects meta-analysis, so the above R code and METAL produce exactly the same results.
