...
CpG, adjusting for sex, age, cell composition, genetic ancestry, and surrogate variables.
...
# wrote, 756,251 probes x 86 samples. It is ~510 MB on disk and about the same
# again in memory, so this read takes several seconds.
Mcb <- readRDS("data/05_mvals_combat.rds")

# Its small companion, which is committed to the repository: the 86-sample
# phenotype table (which carries the genetic ancestry PCs pc1-pc3), the six cell
# proportions from chapter 04, the smoking proxy, and the surrogate variables
# with the k that chapter 05's sweep settled on.
sv_pieces <- readRDS("data/05_sva.rds")
mdk   <- sv_pieces$mdk            # phenotype table (incl. pc1-pc3), rows in Mcb's column order
...
- **genetic ancestry** — the three genetic ancestry PCs (`pc1`–`pc3`) deposited with
  the series, introduced in [chapter 05](05_batch_effects.qmd).
...
Chapters 01–04 worked with all **96** QC-passing arrays, because none of those steps
needs the exposure. The batch correction and surrogate variables in
[chapter 05](05_batch_effects.qmd) do, so that chapter already set aside the nine
samples with no recorded PTSD status and the one sample with no genetic ancestry PCs.
The matrix loaded above is therefore the same modeled set, and the EWAS
...
read it instead of running the chunks that fit and save the model: `ewas-fit` below,
and `run-bacon`, `bacon-summary`, `bacon-diagnostics` and `save-ewas` in
[section 4](#sec-bacon) and at the end of the chapter:
...
Every other number, table and figure in this chapter will then run. The four BACON
diagnostic plots are the one thing you lose, because they are drawn from the `bc` object
rather than from the saved table.
:::
...
# pc1-pc3 are the genetic ancestry PCs carried on mdk from chapter 05.
dd <- data.frame(ptsd = ptsd, sex = factor(mdk$sex), age = mdk$age,
                 smoke = smoke, pos = pos,
                 pc1 = mdk$pc1, pc2 = mdk$pc2, pc3 = mdk$pc3,
                 propk, SV = SV, check.names = FALSE)
design <- model.matrix(~ ptsd + sex + age + smoke + pos + pc1 + pc2 + pc3 +
...
# 756,251 x 86 doubles, so it is dropped again as soon as the means are taken.
bcb <- 2^Mcb / (2^Mcb + 1)
...
And it is not a substitute for a better model: if your λ is inflated because of
unmodeled batch or cell composition, the right response is to fix that (chapters 04 and
05) rather than lean on BACON. Use it for the residual bias that remains after
adjustment.
:::
...
`r n_bonf` CpGs pass Bonferroni and `r n_fdr` pass FDR < 0.05 — a null
result that a study of this size cannot settle either way. A null result is not unusual at this sample size. The Grady Trauma Project was one of 23 cohorts in the Psychiatric Genomics Consortium PTSD meta-analysis of blood DNA methylation, which with 5,077 participants (2,156 PTSD cases and 2,921 trauma-exposed controls) identified 11 CpGs associated with PTSD [@katrinli2024ptsd].

The next code blocks produce a Manhattan plot, a QQ plot and a volcano plot from the BACON-adjusted results. No CpG reaches epigenome-wide significance: the strongest, `cg15434749`, has a BACON-adjusted p = 2.01 × 10⁻⁷ against a Bonferroni threshold of 6.61 × 10⁻⁸, and an FDR of 0.090.

A Manhattan plot needs a genomic position for every CpG. Here they come from Zhou's
hg38 coordinates, the same annotation chapter 08 uses:

```{r}
#| label: manhattan-coords
library(data.table)   # fread(), and the table operations below
library(ggplot2)      # the Manhattan, QQ and volcano plots

# hg38 position of every EPIC probe. The coordinate table has no probe column: row i
# belongs to the probe named on row i of the probe ordering, so the two are read
# together. The ordering is the file chapter 03 committed; the coordinates (5 MB)
# are read from the zhou-lab/InfiniumAnnotation repository.
ord <- fread("data/EPIC.ordering.tsv.gz", select = "Probe_ID")
...
coords <- data.table(probe = ord$Probe_ID, chr = crd$CpG_chrm, pos = crd$CpG_beg)

# One row per tested CpG, with its position and its BACON-adjusted p-values
pl <- merge(as.data.table(tt)[, .(probe, bacon.p, bacon.adj.P)], coords,
            by = "probe")

# Keep the numbered chromosomes. Chapter 03 removed chrX and chrY; this also drops
# the 17 tested CpGs that sit on unplaced contigs or have no hg38 position.
pl <- pl[grepl("^chr[0-9]+$", chr)]
pl[, chrn := as.integer(sub("^chr", "", chr))]
setorder(pl, chrn, pos)

# Lay the chromosomes end to end: each chromosome's positions are shifted right by
# the combined length of the chromosomes before it.
chr_len <- pl[, .(len = max(pos)), by = chrn]
chr_len[, offset := cumsum(as.numeric(len)) - len]
pl[chr_len, x := pos + i.offset, on = "chrn"]
chr_mid <- pl[, .(mid = (min(x) + max(x)) / 2), by = chrn]   # where each axis label goes

# One function draws the plot; `pcol` names the p-value column it is drawn on.
# `bonf` is the Bonferroni threshold from the hits chunk above.
manhattan <- function(d, pcol, title) {
  ggplot(d, aes(x, -log10(.data[[pcol]]), colour = factor(chrn %% 2))) +
    geom_point(size = 0.8, alpha = 0.5) +
    geom_hline(yintercept = -log10(bonf), colour = "#8C3A4A") +          # plum
    scale_colour_manual(values = c(`0` = "#0F3D43", `1` = "#2A8F9B"),    # alternating teal
                        guide = "none") +
    scale_x_continuous(breaks = chr_mid$mid, labels = chr_mid$chrn,
                       expand = c(0.01, 0)) +
...
#| fig-width: 11
#| fig-height: 5
#| fig-cap: "Manhattan plot of the EWAS, drawn on the BACON-adjusted p-values — the ones the chapter recommends you report. No CpG crosses the Bonferroni line (plum); the top-ranked CpG, `cg15434749`, is labelled. A near-empty Manhattan is the expected result for a subtle psychiatric phenotype in a small teaching subset (n=86); the full Grady Trauma Project cohort (n≈800) and meta-analysis across cohorts is what powers genome-wide discovery for PTSD [@katrinli2024ptsd]."
top1 <- pl[which.min(bacon.p)]                      # the top-ranked CpG, labelled

manhattan(pl, "bacon.p", "PTSD EWAS, BACON-adjusted p-values") +
  geom_text(data = top1, aes(x, -log10(bacon.p), label = probe),
            inherit.aes = FALSE, vjust = -1.2, size = 3.3, colour = "#3A362F")
```

A QQ plot summarises the same BACON-adjusted results across all CpGs at once. BACON's own version is among the
diagnostics above. Code for producing your own qq plots is provided here so you can make them for your own studies.  Each CpG's observed
p-value is plotted against the p-value it would have under a null distribution.

```{r}
#| label: qq
#| fig-width: 6
#| fig-height: 6
#| fig-cap: "QQ plot of the BACON-adjusted EWAS p-values. Points on the diagonal are what a study with no associations produces; a curve lifting off the line across the whole range is inflation, while a departure confined to the far right is candidate signal. The genomic inflation factor λ is in the subtitle. The curve follows the diagonal through the bulk of the distribution and lies slightly above it in the upper tail; the most extreme CpG reaches −log₁₀ p = 6.70, below the Bonferroni threshold of 7.18."
n <- nrow(tt)
qq <- data.table(expected = -log10(ppoints(n)),
                 observed = -log10(sort(tt$bacon.p)))

ggplot(qq, aes(expected, observed)) +
  geom_abline(slope = 1, intercept = 0, colour = "#7F7F7F") +
  geom_point(size = 0.8, colour = "#2A8F9B") +
  labs(x = expression(Expected~-log[10](italic(p))),
       y = expression(Observed~-log[10](italic(p))),
       subtitle = sprintf("BACON-adjusted p-values (\u03bb = %.3f)", bs$lambda_bacon))
```
...
#| fig-width: 7
#| fig-height: 6
#| fig-cap: "Volcano plot. The x-axis is the EWAS effect size itself — the `ptsdCase` coefficient from the limma fit (logFC), on the M-value scale, adjusted for every covariate in the design; the y-axis is the BACON-adjusted p-value. Colour marks the sign of that coefficient: dark teal for CpGs less methylated in cases, light teal for those more methylated. Note that this is the adjusted effect, so its sign can differ from a raw case-minus-control β difference for probes where the covariates carry part of the signal. The plum line is the Bonferroni threshold, which no CpG crosses. The three top-ranked CpGs are ringed and labelled: `cg15434749` (logFC −0.385, Δβ −0.026), `cg20600436` (logFC 0.171, Δβ 0.014) and `cg25717994` (logFC 0.310, Δβ 0.006). Even they move methylation only slightly, illustrating that EWAS effect sizes for complex phenotypes are typically small and require large samples to detect reliably."
library(ggrepel)      # geom_text_repel(): keeps the three labels from overlapping

# x: the tested ptsdCase coefficient (logFC, M-value scale); y: BACON-adjusted p.
vd <- as.data.table(tt)[, .(probe, logFC, bacon.p)]
vd[, direction := ifelse(logFC < 0, "Less methylated in cases",
                                    "More methylated in cases")]
top3 <- vd[order(bacon.p)][1:3]                     # the three top-ranked CpGs

ggplot(vd, aes(logFC, -log10(bacon.p), colour = direction)) +
  geom_point(size = 0.6, alpha = 0.4) +
  geom_hline(yintercept = -log10(bonf), colour = "#8C3A4A") +          # plum
  geom_point(data = top3, shape = 21, fill = NA, colour = "#8C3A4A",
             size = 3.5, stroke = 1) +
  geom_text_repel(data = top3, aes(label = probe), colour = "#3A362F",
                  size = 3.3, nudge_y = 0.4, min.segment.length = 0,
                  segment.colour = "#3A362F", seed = 1) +
  scale_colour_manual(values = c(`Less methylated in cases` = "#0F3D43",
                                 `More methylated in cases` = "#2A8F9B"),
                      name = NULL) +
  labs(x = "PTSD effect (logFC, M-value scale)",
...
2. **Run the association on the residuals** with only the biological model:
   $$\text{resid}(\text{M})_{\text{CpG}} \sim \text{PTSD} + \text{sex} + \text{age} + \text{smoking proxy} + \text{cell composition} + \text{ancestry PCs}$$

```{.r filename="Generic example, do not run"}
# Schematic only -- placeholder names, not objects this chapter creates.
# Stage 1: residualize M-values on the technical terms (SVs here; or chip + position)
tech    <- model.matrix(~ SV)                      # nuisance-only design
M_resid <- residuals(lmFit(mvals, tech), mvals)    # limma residuals, probes x samples

# Stage 2: association model on the residuals, biological covariates only
bio <- model.matrix(~ exposure + sex + age + smoking + cell_proportions + ancestry_pcs, data = pheno)
...
(`PTSD + sex + age + smoking proxy + 6 cell props + 3 ancestry PCs`) and returns df to the test that matters.
...
The standard tool for performing meta analysis in genomics is **METAL**
[@willer2010metal] and can also be used for epigenomics.  It reads each
stratum's results file, matches CpGs by ID, does the inverse-variance weighting and the
heterogeneity test genome-wide, and writes a combined table. [Chapter 07](07_pipeline.qmd) runs the stratified EWAS and METAL as a
managed workflow. METAL's `SCHEME STDERR` *is* an inverse-variance-weighted fixed-effects meta-analysis, so the R code below and METAL produce exactly the same results.

Inverse-variance meta-analysis is short enough to show in full. Every line below is
vectorized, so this is the whole genome-wide meta-analysis, not a one-CpG illustration:

```{.r filename="Generic example, do not run"}
# Schematic only -- the arithmetic, with the inputs named rather than built.
# resF and resM: each stratum's BACON-adjusted results, one row per CpG, with
# columns probe, effect and se. Align them on the CpGs tested in both strata;
# everything after this line is vectorized over all of them at once.
m   <- merge(resF, resM, by = "probe", suffixes = c("F", "M"))
bF  <- m$effectF;  seF <- m$seF          # female effect and SE, one per CpG
bM  <- m$effectM;  seM <- m$seM          # male effect and SE
...
I2    <- pmax(0, (Q - 1) / Q) * 100           # % of between-stratum variance

# direction string, the way METAL reports it: one sign per stratum
direction <- paste0(ifelse(bF > 0, "+", "-"), ifelse(bM > 0, "+", "-"))

meta <- data.frame(probe = m$probe, beta_meta, se_meta, p_meta, p_het, I2, direction)
meta <- meta[order(meta$p_meta), ]
```

The direction and heterogeneity columns are interpretive rather than a quality filter.
Concordance strengthens a combined estimate; discordance can point to a genuine
stratum-specific effect (in this case discordance could indicate sex-specific effects),
which is part of why you stratified. In a multi-cohort meta-analysis, where the strata
are meant to be replicates, the discordance would indicate lower credibility.

Two things follow for how you read a meta-analysed table:

1. **Check the arms.** For every hit you plan to report, look up the per-stratum
   estimate, SE and p-value. If one arm is flat, you have a stratum-specific finding
   and you should describe it that way.
2. **Distrust `I²` at two strata.** With two studies and one heterogeneity degree of
   freedom, I² is extremely noisy. A CpG whose two estimates point in opposite
   directions can still report I² = 0 if one arm's SE is large enough that the two
   are statistically compatible — low I² there means "not enough precision to tell
   them apart", not "they agree".

::: {.callout-tip}
## Why use METAL?
METAL is the commonly used tool for meta-analysis of genome-wide association studies. METAL handles allele/strand alignment
(irrelevant for methylation), reads many cohort files with
